  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold text-ink leading-tight">
            <Link href={`/job?id=${encodeURIComponent(job.id)}`} className="hover:underline">
              {job.title}
            </Link>
          </h3>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <Link className="px-3 py-2 rounded-md border border-border focus-ring" href={`/job?id=${encodeURIComponent(job.id)}`}>Details</Link>
      </div>
    </article>
  );
}
